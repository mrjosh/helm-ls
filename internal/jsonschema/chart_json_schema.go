...
func (g *SchemaGenerator) generateSchemaForRelatedChart(scopedValuesfiles *charts.ScopedValuesFiles) {
	...
	if len(scopedValuesfiles.SubScope) > 0 {
		...
		for _, v := range scopedValuesfiles.SubScope {
			refPointer = fmt.Sprintf("%s/properties/%s", refPointer, escapeJSONPointer(v))
		}
	...
	for _, refPointer := range refPointers {
		ref := fmt.Sprintf("%s#/$defs/%s%s",
			schemFilePath,
			escapeJSONPointer(scopedValuesfiles.Chart.Name()),
			refPointer,
		)
...
func (g *SchemaGenerator) getDescriptionForValuesSchema(valuesFile *charts.ValuesFile) string {
	return fmt.Sprintf("%s for the chart %s using values from the file %s", DescriptionPrefix, g.chart.Name(), filepath.Base(valuesFile.URI.Filename()))
}

func (g *SchemaGenerator) getDescriptionForGlobalValues(valuesFile *charts.ValuesFile) string {
	return fmt.Sprintf("%s for the chart %s using global values from the file %s", DescriptionPrefix, g.chart.Name(), filepath.Base(valuesFile.URI.Filename()))
}
...
func (g *SchemaGenerator) addDef(name string, schema *Schema) {
	...
	g.allOf = append(g.allOf, &Schema{
		Ref: fmt.Sprintf("#/$defs/%s", escapeJSONPointer(name)),
	})
}
...
func escapeJSONPointer(pointer string) string {
